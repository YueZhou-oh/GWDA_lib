...
class PosteriorModel(object):
    def __init__(self, model_dir=None, data_dir=None, basis_dir=None, sample_extrinsic_only=True, save_aux_filename="waveforms_supplementary.hdf5", save_model_name="model.pt", use_cuda=True):
        self.wfd = None
        self.model = None
    ...
    def load_dataset(self, batch_size=512, detectors=None, truncate_basis=None, snr_threshold=None, distance_prior_fn=None, distance_prior=None, sampling_from=None, nsample=10000, nsamples_target_event=0, mixed_alpha=None, bw_dstar=None):
        ...
        if self.data_dir is None:
            raise NameError("Data directory must be specified." " Store in attribute PosteriorModel.data_dir")
        ...
        if self.wfd.sampling_from == "posterior":
            ...
            self.wfd.parameters = np.concatenate((parameters_posterior[: int(nsample * mixed_alpha)], parameters_uniform[: (nsample - int(nsample * mixed_alpha))]), axis=0)
            self.wfd.nsamples = len(self.wfd.parameters)
        ...
        # DataLoader objects
        self.train_loader = DataLoader(wfd_train, batch_size=batch_size, shuffle=False, pin_memory=True, num_workers=16, worker_init_fn=lambda _: np.random.seed(int(torch.initial_seed()) % (2**32 - 1)))
        self.test_loader = DataLoader(wfd_test, batch_size=batch_size, shuffle=False, pin_memory=True, num_workers=16, worker_init_fn=lambda _: np.random.seed(int(torch.initial_seed()) % (2**32 - 1)))
    ...
    def construct_model(self, model_type, existing=False, **kwargs):
        ...
        if not existing:
            ...
            self.model = model_creator(input_dim=input_dim, context_dim=context_dim, **kwargs)
        else:
        ...
        if model_type == "maf" or model_type == "cvae":
            self.base_dist = torch.distributions.MultivariateNormal(loc=torch.zeros(base_dim, device=self.device), covariance_matrix=torch.diag_embed(torch.ones(base_dim, device=self.device)))
    ...
    def initialize_training(self, lr=0.0001, lr_annealing=True, anneal_method="step", total_epochs=None, steplr_step_size=80, steplr_gamma=0.5, flow_lr=None):
        ...
        if (flow_lr is not None) and (self.model_type == "cvae"):
            param_list = [{"params": self.model.encoder.parameters()}, {"params": self.model.decoder.parameters()}, {"params": self.model.prior_nn.parameters()}]
            possible_flows = [self.model.iaf, self.model.prior_maf, self.model.decoder_maf]
            for flow in possible_flows:
                if flow is not None:
                    param_list.append({"params": flow.parameters(), "lr": flow_lr})
    ...
    def save_model(self, filename="model.pt", aux_filename="waveforms_supplementary.hdf5"):
        ...
        if self.model_dir is None:
            raise NameError("Model directory must be specified." " Store in attribute PosteriorModel.model_dir")

        p = Path(self.model_dir)
        p.mkdir(parents=True, exist_ok=True)

        dict = {"model_type": self.model_type, "model_hyperparams": self.model.model_hyperparams, "model_state_dict": self.model.state_dict(), "optimizer_state_dict": self.optimizer.state_dict(), "epoch": self.epoch, "detectors": self.detectors}
        ...
        if self.wfd.domain == "RB":
            ...
            for (
                ifo,
                std,
            ) in self.wfd.basis.standardization_dict.items():
    ...
    def load_model(self, filename="model.pt"):
        ...
        if len(checkpoint["optimizer_state_dict"]["param_groups"]) > 1:
            ...
            flow_lr = None
        self.initialize_training(lr_annealing=scheduler_present_in_checkpoint, flow_lr=flow_lr)
    ...
    def train(self, epochs, output_freq=50, kl_annealing=True, snr_annealing=False):
        """Train the model.
        ...
        for epoch in range(self.epoch, self.epoch + epochs):
            ...
            if self.model_type == "maf":
                train_loss = a_flows.train_epoch(self.model, self.base_dist, self.train_loader, self.optimizer, epoch, self.device, output_freq)
                a_flows.recalculate_moving_avgs(self.model, self.train_loader, self.device)
                test_loss = a_flows.test_epoch(self.model, self.base_dist, self.test_loader, self.device)

            elif self.model_type == "nde":
                train_loss = nde_flows.train_epoch(self.model, self.train_loader, self.optimizer, epoch, self.device, output_freq, add_noise, snr_annealing)
                test_loss = nde_flows.test_epoch(self.model, self.test_loader, epoch, self.device, add_noise, snr_annealing)

            elif self.model_type == "cvae":
                train_loss, train_reconstruction_loss, train_kl_loss = cvae.train_epoch(self.model, self.base_dist, self.train_loader, self.optimizer, epoch, self.device, output_freq, annealing=kl_annealing)
                cvae.recalculate_moving_avgs(self.model, self.train_loader, self.base_dist, self.device)
                test_loss, test_reconstruction_loss, test_kl_loss = cvae.test_epoch(self.model, self.base_dist, self.test_loader, epoch, self.device, annealing=kl_annealing)
                ...
                if epoch == 1:
                    ...
                    if self.model_type == "cvae":
                        with open(p / "kl_history.txt", "w") as f:
                            writer = csv.writer(f, delimiter="\t")
                            writer.writerow([epoch, train_kl_loss, test_kl_loss])
                else:
                ...
                if (output_freq is not None) and (epoch == epoch_minimum_test_loss):
                    for f in os.listdir(p):
                        if "_model.pt" in f:
                            ...
                            os.remove(p / f)
                    print("Saving model as e{}_{} & e{}_{}".format(epoch, self.save_model_name, epoch, self.save_aux_filename))
                    self.save_model(filename="e{}_".format(epoch) + self.save_model_name, aux_filename="e{}_".format(epoch) + self.save_aux_filename)
    ...
    def evaluate(self, idx, nsamples=10000, plot=True):
        ...
        if self.wfd.noisy_test_waveforms is None:
            ...
            if list(self.wfd.detectors.keys()) != self.detectors:
                raise Exception("Model trained on different number of " "detectors than contained in test data.")
        ...
        if self.model_type == "maf":
            x_samples = a_flows.obtain_samples(self.model, self.base_dist, y, nsamples, self.device)
        elif self.model_type == "nde":
            x_samples = nde_flows.obtain_samples(self.model, y, nsamples, self.device)
        elif self.model_type == "cvae":
            x_samples = cvae.obtain_samples(self.model, self.base_dist, y, nsamples, self.device)
        ...
        if plot:
            corner.corner(params_samples, truths=params_true, labels=self.wfd.parameter_labels)
            plt.show()
...
def parse_args():
    parser = argparse.ArgumentParser(description=("Model the gravitational-wave parameter " "posterior distribution with neural networks."))
    ...
    dir_parent_parser.add_argument("--nsamples_target_event", type=int, default="0")
    dir_parent_parser.add_argument("--mixed_alpha", type=float, default="0.0")
    dir_parent_parser.add_argument("--nsample", type=int, default="1000000")

    activation_parent_parser = argparse.ArgumentParser(add_help=None)
    activation_parent_parser.add_argument("--activation", choices=["relu", "leaky_relu", "elu"], default="relu")

    train_parent_parser = argparse.ArgumentParser(add_help=None)
    train_parent_parser.add_argument("--batch_size", type=int, default="512")
    train_parent_parser.add_argument("--lr", type=float, default="0.0001")
    train_parent_parser.add_argument("--lr_anneal_method", choices=["step", "cosine", "cosineWR"], default="step")
    train_parent_parser.add_argument("--no_lr_annealing", action="store_false", dest="lr_annealing")
    train_parent_parser.add_argument("--steplr_gamma", type=float, default=0.5)
    train_parent_parser.add_argument("--steplr_step_size", type=int, default=80)
    ...
    train_parent_parser.add_argument("--output_freq", type=int, default="50")
    train_parent_parser.add_argument("--no_save", action="store_false", dest="save")
    train_parent_parser.add_argument("--no_kl_annealing", action="store_false", dest="kl_annealing")
    ...
    train_parent_parser.add_argument("--distance_prior_fn", choices=["uniform_distance", "inverse_distance", "linear_distance", "inverse_square_distance", "bayeswave"])
    train_parent_parser.add_argument("--snr_annealing", action="store_true")
    train_parent_parser.add_argument("--distance_prior", type=float, nargs=2)
    train_parent_parser.add_argument("--bw_dstar", type=float)

    cvae_parent_parser = argparse.ArgumentParser(add_help=False)
    cvae_parent_parser.add_argument("--latent_dim", type=int, required=True)
    cvae_parent_parser.add_argument("--hidden_dims", type=int, nargs="+", required=True)
    cvae_parent_parser.add_argument("--batch_norm", action="store_true")
    cvae_parent_parser.add_argument("--prior_gaussian_nn", action="store_true")
    cvae_parent_parser.add_argument("--prior_full_cov", action="store_true")

    iaf_parent_parser = argparse.ArgumentParser(add_help=False)
    iaf_parent_parser.add_argument("--iaf.hidden_dims", type=int, nargs="+", required=True)
    context_group = iaf_parent_parser.add_mutually_exclusive_group(required=True)
    ...
    iaf_parent_parser.add_argument("--iaf.bn_momentum", type=float, default=0.9)
    iaf_parent_parser.add_argument("--iaf.maf_parametrization", action="store_false", dest="iaf.iaf_parametrization")
    ...
    maf_prior_parent_parser.add_argument("--maf_prior.hidden_dims", type=int, nargs="+", required=True)
    maf_prior_parent_parser.add_argument("--maf_prior.nflows", type=int, required=True)
    maf_prior_parent_parser.add_argument("--maf_prior.no_batch_norm", action="store_false", dest="maf_prior.batch_norm")
    maf_prior_parent_parser.add_argument("--maf_prior.bn_momentum", type=float, default=0.9)
    maf_prior_parent_parser.add_argument("--maf_prior.iaf_parametrization", action="store_true")

    maf_decoder_parent_parser = argparse.ArgumentParser(add_help=False)
    maf_decoder_parent_parser.add_argument("--maf_decoder.hidden_dims", type=int, nargs="+", required=True)
    maf_decoder_parent_parser.add_argument("--maf_decoder.nflows", type=int, required=True)
    maf_decoder_parent_parser.add_argument("--maf_decoder.no_batch_norm", action="store_false", dest="maf_decoder.batch_norm")
    maf_decoder_parent_parser.add_argument("--maf_decoder.bn_momentum", type=float, default=0.9)
    maf_decoder_parent_parser.add_argument("--maf_decoder.iaf_parametrization", action="store_true")
    maf_decoder_parent_parser.add_argument("--maf_decoder.zcontext", action="store_true")
    ...
    train_new_parser = train_subparsers.add_parser("new", description=("Build and train a network."))
    ...
    maf_parser = type_subparsers.add_parser("maf", description=("Build and train a MAF."), parents=[activation_parent_parser, dir_parent_parser, train_parent_parser])
    maf_parser.add_argument("--hidden_dims", type=int, nargs="+", required=True)
    maf_parser.add_argument("--nflows", type=int, required=True)
    maf_parser.add_argument("--no_batch_norm", action="store_false", dest="batch_norm")
    ...
    nde_parser = type_subparsers.add_parser("nde", description=("Build and train a flow from the nde package."), parents=[activation_parent_parser, dir_parent_parser, train_parent_parser])
    ...
    nde_parser.add_argument("--apply_unconditional_transform", action="store_true")
    ...
    nde_parser.add_argument("--base_transform_type", type=str, choices=["rq-coupling", "rq-autoregressive"], default="rq-coupling")

    # Pure CVAE

    cvae_parser = type_subparsers.add_parser("cvae", description=("Build and train a CVAE."), parents=[activation_parent_parser, dir_parent_parser, cvae_parent_parser, train_parent_parser])
    cvae_parser.add_argument("--encoder_diag_cov", action="store_false", dest="encoder_full_cov")
    cvae_parser.add_argument("--decoder_diag_cov", action="store_false", dest="decoder_full_cov")

    # CVAE with IAF

    cvae_iaf_parser = type_subparsers.add_parser("cvae+iaf", description=("Build and train a CVAE with IAF encoder."), parents=[activation_parent_parser, dir_parent_parser, cvae_parent_parser, iaf_parent_parser, train_parent_parser])
    cvae_iaf_parser.add_argument("--encoder_full_cov", action="store_true")
    cvae_iaf_parser.add_argument("--decoder_diag_cov", action="store_false", dest="decoder_full_cov")

    # CVAE with prior MAF

    cvae_maf_prior_parser = type_subparsers.add_parser(
        "cvae+maf_prior", description=("Build and train a CVAE with MAF prior."), parents=[activation_parent_parser, dir_parent_parser, cvae_parent_parser, maf_prior_parent_parser, train_parent_parser]
    )
    cvae_maf_prior_parser.add_argument("--encoder_full_cov", action="store_true")
    cvae_maf_prior_parser.add_argument("--decoder_diag_cov", action="store_false", dest="decoder_full_cov")

    # CVAE with decoder MAF

    cvae_maf_decoder_parser = type_subparsers.add_parser(
        "cvae+maf_decoder", description=("Build and train a CVAE with MAF decoder."), parents=[activation_parent_parser, dir_parent_parser, cvae_parent_parser, maf_decoder_parent_parser, train_parent_parser]
    )
    cvae_maf_decoder_parser.add_argument("--encoder_diag_cov", action="store_false", dest="encoder_full_cov")
    cvae_maf_decoder_parser.add_argument("--decoder_full_cov", action="store_true")

    # CVAE with IAF + MAF decoder

    cvae_iaf_maf_decoder_parser = type_subparsers.add_parser(
        "cvae+iaf+maf_decoder", description=("Build and train a CVAE with IAF encoder" "and MAF decoder."), parents=[activation_parent_parser, dir_parent_parser, cvae_parent_parser, iaf_parent_parser, maf_decoder_parent_parser, train_parent_parser]
    )
    cvae_iaf_maf_decoder_parser.add_argument("--encoder_full_cov", action="store_true")
    cvae_iaf_maf_decoder_parser.add_argument("--decoder_full_cov", action="store_true")

    # CVAE with prior MAF + posterior MAF

    cvae_maf_prior_maf_decoder_parser = type_subparsers.add_parser(
        "cvae+maf_prior+maf_decoder",
        description=("Build and train a CVAE with MAF prior" "and MAF decoder."),
        parents=[activation_parent_parser, dir_parent_parser, cvae_parent_parser, maf_prior_parent_parser, maf_decoder_parent_parser, train_parent_parser],
    )
    cvae_maf_prior_maf_decoder_parser.add_argument("--encoder_full_cov", action="store_true")
    cvae_maf_prior_maf_decoder_parser.add_argument("--decoder_full_cov", action="store_true")

    # CVAE with IAF + prior MAF + posterior MAF

    cvae_all_parser = type_subparsers.add_parser(
        "cvae+all",
        description=("Build and train a CVAE with IAF, MAF prior" "and MAF decoder."),
        parents=[activation_parent_parser, dir_parent_parser, cvae_parent_parser, iaf_parent_parser, maf_prior_parent_parser, maf_decoder_parent_parser, train_parent_parser],
    )
    cvae_all_parser.add_argument("--encoder_full_cov", action="store_true")
    cvae_all_parser.add_argument("--decoder_full_cov", action="store_true")

    train_subparsers.add_parser("existing", description=("Load a network from file and continue training."), parents=[dir_parent_parser, train_parent_parser])
...
def main():
    ...
    if args.mode == "train":
        ...
        pm = PosteriorModel(
            model_dir=args.model_dir, data_dir=args.data_dir, basis_dir=args.basis_dir, sample_extrinsic_only=args.dont_sample_extrinsic_only, save_model_name=args.save_model_name, save_aux_filename=args.save_aux_filename, use_cuda=args.cuda
        )
        ...
        if args.model_source == "new":
            ...
            if args.model_type == "maf":
                pm.construct_model("maf", hidden_dims=args.hidden_dims, nflows=args.nflows, batch_norm=args.batch_norm, bn_momentum=args.bn_momentum, iaf_parametrization=args.iaf_parametrization, activation=args.activation)
                ...
                pm.construct_model(
                    ...
                    prior_maf={
                        ...
                        "iaf_parametrization": args.maf_prior.iaf_parametrization,
                    },
                    batch_norm=args.batch_norm,
                    prior_gaussian_nn=args.prior_gaussian_nn,
                    prior_full_cov=args.prior_full_cov,
                ...
                pm.construct_model(
                    ...
                    decoder_maf={
                        ...
                        "iaf_parametrization": args.maf_decoder.iaf_parametrization,
                    },
            ...
            if args.lr_annealing is True:
                if args.lr_anneal_method == "step":
                    print("Stepping learning rate by", args.steplr_gamma, "every", args.steplr_step_size, "epochs")
        ...
        if args.transfer_epochs:
            ...
            try:
                pm.train(args.transfer_epochs, output_freq=args.output_freq, kl_annealing=args.kl_annealing, snr_annealing=args.snr_annealing)
            except KeyboardInterrupt as e:
                print(e)
            finally:
                print("Stopping timer.")
                stop_time = time.time()
                print("Training time (including validation): {} seconds".format(stop_time - start_time))

                if args.save:
                    print("Saving model")
                    pm.save_model(filename="a{}".format(args.mixed_alpha) + pm.save_model_name, aux_filename="a{}".format(args.mixed_alpha) + pm.save_aux_filename)
            ...
            for i, mixed_alpha in enumerate(alpha_list):
                ...
                try:
                    ...
                    if args.save:
                        print("Saving model")
                        pm.save_model(filename="a{}".format(mixed_alpha) + pm.save_model_name, aux_filename="a{}".format(mixed_alpha) + pm.save_aux_filename)
        else:  # You should set args.mixed_alpha = 1.0
            try:
                pm.train(args.epochs, output_freq=args.output_freq, kl_annealing=args.kl_annealing, snr_annealing=args.snr_annealing)
            except KeyboardInterrupt as e:
                print(e)
            finally:
                print("Stopping timer.")
