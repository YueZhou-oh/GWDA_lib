...
def compute_mask(shape, sample_lens, mask_prob, mask_length):
    ...
    num_mask = int(mask_prob * min_sample_len / float(mask_length) + random.random() + 1)
    ...
    for i in range(bs):
        ...
        mask_indices = np.random.choice(sample_len - mask_length, num_mask, replace=False)

        # Now using the starting indices create contiguous masks.
        mask_indices = np.asarray([mask_indices[j] + offset for j in range(len(mask_indices)) for offset in range(mask_length)])
