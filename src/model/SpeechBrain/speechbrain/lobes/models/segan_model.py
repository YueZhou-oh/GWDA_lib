...
class Generator(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.latent_vae:
            ...
            x = z_mean + torch.exp(z_logvar / 2.0) * torch.randn_like(z_logvar, device=x.device)  # sampling from latent var probability distribution
        elif self.z_prob:
    ...
    def __init__(self, kernel_size):
        ...
        for i in range(len(Channels) - 1):
            if i != len(Channels) - 2:
                ...
                self.Norms.append(nn.BatchNorm1d(num_features=Channels[i + 1]))  # not sure what the last dim should be here
            # output convolution
            else:
                self.Layers.append(
                    nn.Conv1d(
...
def g3_loss(
    ...
    if not (z_mean is None):  # This will determine if model is being trained as a vae
        ...
        distq = torch.distributions.normal.Normal(z_mean, torch.exp(z_logvar) ** (1 / 2))
