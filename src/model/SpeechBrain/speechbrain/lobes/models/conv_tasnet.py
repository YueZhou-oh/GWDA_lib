...
class Decoder(nn.Module):
    ...
    def __init__(self, L, N):
        ...
        # Components
        self.basis_signals = sb.nnet.linear.Linear(input_size=N, n_neurons=L, bias=False)
    ...
    def forward(self, mixture_w, est_mask):
        ...
        """
        # D = W * M
        source_w = torch.unsqueeze(mixture_w, 2).repeat(1, 1, est_mask.size(2), 1) * est_mask  # [M, K, C, N]
...
class MaskNet(nn.Module):
    ...
    def forward(self, mixture_w):
        ...
        score = score.contiguous().reshape(M, K, self.C, N)  # [M, K, C*N] -> [M, K, C, N]
