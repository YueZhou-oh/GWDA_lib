...
class OrionHyperparameterOptimizationReporter(HyperparameterOptimizationReporter):
    ...
    def trial_id(self):
        """The unique ID of this trial (used mainly for folder naming)"""
        if self._trial_id is None:
            self._trial_id = "-".join(os.getenv(name) or "" for name in ORION_TRIAL_ID_ENV)
        return self._trial_id
    ...
    def is_available(self):
        ...
        return self.orion_client is not None and any(os.getenv(name) for name in ORION_TRIAL_ID_ENV)
...
class HyperparameterOptimizationContext:
    ...
    def parse_arguments(self, arg_list):
        ...
        if hpopt:
            self.enabled = True
            self.reporter = get_reporter(hpopt_mode, *self.reporter_args, **self.reporter_kwargs)
