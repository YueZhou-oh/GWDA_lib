...
def multiprocess_evaluation(metric, predict, target, lengths=None, n_jobs=8):
    ...
    while True:
        try:
            scores = Parallel(n_jobs=n_jobs, timeout=30)(delayed(metric)(p, t) for p, t in zip(predict, target))
            break
...
class BinaryMetricStats(MetricStats):
    ...
    def summarize(self, field=None, threshold=None, max_samples=None, beta=1, eps=1e-8):
        ...
        if threshold is None:
            positive_scores = self.scores[(self.labels == self.positive_label).nonzero(as_tuple=True)]
            negative_scores = self.scores[(self.labels != self.positive_label).nonzero(as_tuple=True)]
        ...
        self.summary["F-score"] = (1.0 + beta**2.0) * TP / ((1.0 + beta**2.0) * TP + beta**2.0 * FN + FP)

        self.summary["MCC"] = (TP * TN - FP * FN) / ((TP + FP) * (TP + FN) * (TN + FP) * (TN + FN) + eps) ** 0.5
...
class ClassificationStats(MetricStats):
    ...
    def summarize(self, field=None):
        ...
        for stat in ["total", "correct", "accuracy"]:
            self.summary[f"classwise_{stat}"] = {key: key_stats[stat] for key, key_stats in self.summary["classwise_stats"].items()}
        if field is not None:
            return self.summary[field]
        else:
            return self.summary
    ...
    def _compute_accuracy(self):
        return sum(prediction == target for prediction, target in zip(self.predictions, self.targets)) / len(self.ids)

    def _build_lookups(self):
        self._available_keys = self._get_keys()
        self._available_predictions = list(sorted(set(prediction for prediction in self.predictions)))
    ...
    def _compute_confusion_matrix(self):
        confusion_matrix = torch.zeros(len(self._available_keys), len(self._available_predictions))
    ...
    def _compute_classwise_stats(self, confusion_matrix):
        ...
        key_targets = self._available_keys if not self.categories else [target for _, target in self._available_keys]
        correct = torch.tensor([(confusion_matrix[idx, self._predictions_lookup[target]] if target in self._predictions_lookup else 0) for idx, target in enumerate(key_targets)])
        ...
        return {
            ...
            for key, item_total, item_correct, item_accuracy in zip(self._available_keys, total, correct, accuracy)
        }
    ...
    def _get_confusion_entries(self):
        if self.categories:
            result = (((category, target), prediction) for category, target, prediction in zip(self.categories, self.targets, self.predictions))
        else:
    ...
    def _write_classwise_stats(self, filestream):
        ...
        for key in self._available_keys:
            ...
            padded_label = self._pad_to_length(self._format_key_label(key), longest_key_label)
            print(
    ...
    def _write_confusion(self, filestream):
        ...
        longest_prediction = max(len(prediction) for prediction in self._available_predictions)
