...
def get_yaml_var(hparam_file):
    ...
    with open(hparam_file) as f:
        for line in f:
            ...
            if len(line) <= 1 or line.startswith("#") or "speechbrain.utils.parameter_transfer.Pretrainer" in line:
                continue
            ...
            if line.find(":") != -1 or line.find("- !ref") != -1:
                ...
                # subvariavles as " key:")
                if not (var_name[0] == " " or var_name[0] == "\t" or "!!" in line or "!apply" in line):
            ...
            for var in var_lst:
                ...
                if re_match is not None:
                    if re_match.group(1) in var:
                        print("\t\tWARNING: potential inconsistency %s maybe used in %s (or not)." % (var, re_match.group(0)))
                        if var not in detected_var:
                            detected_var.append(var)
                            continue
...
def extract_patterns(lines, start_pattern, end_pattern):
    ...
    for line in lines:
        start_indexes = [index for index in range(len(line)) if line.startswith(start_pattern, index)]
...
def check_module_vars(hparam_file, script_file, module_key="modules:", module_var="self.modules."):
    ...
    for unused_var in unused_vars:
        print('\tERROR: variable "self.modules.%s" used in %s, but not listed in the "modules:" section of %s' % (unused_var, script_file, hparam_file))
    return len(unused_vars) == 0
