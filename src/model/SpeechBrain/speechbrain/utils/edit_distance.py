...
def _batch_stats(refs, hyps):
    ...
    if len(refs) != len(hyps):
        raise ValueError("The reference and hypothesis batches are not of the same size")
    stats = collections.Counter()
...
def wer_details_by_utterance(ref_dict, hyp_dict, compute_alignments=False, scoring_mode="strict"):
    ...
    for key, ref_tokens in ref_dict.items():
        ...
        if key in hyp_dict:
            ...
            raise KeyError("Key " + key + " in reference but missing in hypothesis and strict mode on.")
        else:
        ...
        utterance_details.update(
            {
                "scored": True,
                "hyp_empty": True if len(hyp_tokens) == 0 else False,  # This also works for e.g. torch tensors
...
def wer_summary(details_by_utterance):
    ...
    ins = dels = subs = 0
    num_scored_tokens = num_scored_sents = num_edits = num_erraneous_sents = num_absent_sents = num_ref_sents = 0
...
def wer_details_by_speaker(details_by_utterance, utt2spk):
    ...
    for speaker, spk_dets in details_by_speaker.items():
        ...
        if spk_dets["num_scored_sents"] > 0:
            spk_dets["WER"] = 100.0 * spk_dets["num_edits"] / spk_dets["num_scored_tokens"]
            spk_dets["SER"] = 100.0 * spk_dets["num_erraneous_sents"] / spk_dets["num_scored_sents"]
        else:
            spk_dets["WER"] = None
            spk_dets["SER"] = None
...
def top_wer_spks(details_by_speaker, top_k=10):
    ...
    """
    scored_speakers = [dets for dets in details_by_speaker if dets["num_scored_sents"] > 0]
