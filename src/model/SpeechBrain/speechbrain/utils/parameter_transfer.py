...
class Pretrainer:
    ...
    def collect_files(self, default_source=None):
        ...
        """
        logger.debug(f"Collecting files (or symlinks) for pretraining in {self.collect_in}.")
        ...
        for name in self.loadables:
            ...
            if name in self.paths:
                ...
                raise ValueError(f"Path not specified for '{name}', " "and no default_source given!")
            path = fetch(
                filename=filename,
                source=source,
