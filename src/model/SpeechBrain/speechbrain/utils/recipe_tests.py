...
def check_performance(check_str, output_folder, recipe_id, pattern=r"performance_check=\[(.*?)\]"):
    ...
    if not (os.path.exists(filename)):
        print("\tERROR: The file %s of recipe %s does not exist (needed for performance checks)" % (filename, recipe_id))
    ...
    if len(lines_filt) == 0:
        print("\tERROR: No entries %s in %s (recipe %s). See performance_check entry." % (epoch, filename, recipe_id))
        return False
    ...
    for line in lines_filt:
        ...
        if var_value is None:
            print("\tERROR: The file %s of recipe %s does not contain the variable %s (needed for performance checks)" % (filename, recipe_id, variable))
            return False
        var_value = float(var_value.group(1))
        check = check_threshold(threshold, var_value)

        if not (check):
            print("\tERROR: The variable %s of file %s (recipe %s) violated the specified threshold (%s %s)" % (variable, filename, recipe_id, var_value, threshold))
...
def check_threshold(threshold, value):
    ...
    # Get threshold value:
    th_value = float(threshold.strip().replace("=", "").replace(">", "").replace("<", ""))
...
def run_recipe_tests(
    ...
    for i, recipe_id in enumerate(test_script.keys()):
        ...
        cmd = "python " + test_script[recipe_id] + " " + test_hparam[recipe_id] + " --output_folder=" + output_fold + " " + test_flag[recipe_id] + " " + run_opts
        ...
        # Check return code
        if return_code != 0:
            print("\tERROR: Error in %s. Check %s and %s for more info." % (recipe_id, stderr_file, stdout_file))
            check = False
...
def load_yaml_test(
    ...
    for i, recipe_id in enumerate(test_script.keys()):
        ...
        print("(%i/%i) Checking %s..." % (i + 1, len(test_script.keys()), hparam_file))
