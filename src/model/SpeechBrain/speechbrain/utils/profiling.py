...
def export(
    ...
    tensorboard_handler = profiler.tensorboard_trace_handler(dir_name=dir_name, worker_name=worker_name, use_gzip=use_gzip)
        ...
        def stop_wrapper():
            ...
            if kineto_profiler is not None:
                ...
                if prof.events():  # kineto events are not aggregatable (sticking with parsed kineto events)
                    ...
                    kineto_events = kineto_profiler._parse_kineto_results(kineto_profiler.kineto_results)
                    # add to trace record
                    prof.speechbrain_event_traces.append(deepcopy(kineto_events))
