...
def torch_parameter_transfer(obj, path, device):
    ...
    """
    incompatible_keys = obj.load_state_dict(torch.load(path, map_location=device), strict=False)
    for missing_key in incompatible_keys.missing_keys:
        logger.warning(f"During parameter transfer to {obj} loading from " + f"{path}, the transferred parameters did not have " + f"parameters for the key: {missing_key}")
    for unexpected_key in incompatible_keys.unexpected_keys:
        logger.warning(f"During parameter transfer to {obj} loading from " + f"{path}, the object could not use the parameters loaded " + f"with the key: {unexpected_key}")
...
class Checkpointer:
    ...
    def add_recoverable(self, name, obj, custom_load_hook=None, custom_save_hook=None):
    ...
    def save_checkpoint(self, meta={}, end_of_epoch=True, name=None, verbosity=logging.INFO):
        """Saves a checkpoint.
        ...
        saved_meta = self._save_checkpoint_metafile(ckpt_dir / METAFNAME, meta, end_of_epoch)
        saved_paramfiles = {}
    ...
    def save_and_keep_only(
        ...
        None
            ...
            deletion.
        """
        self.save_checkpoint(meta=meta, end_of_epoch=end_of_epoch, name=name, verbosity=verbosity)
    ...
    def find_checkpoints(
        ...
        if max_key and not importance_key:
            ...
            raise ValueError("Must specify only one of 'importance_key', 'max_key', " "and 'min_key'.")
    ...
    def delete_checkpoints(
        ...
        *,
        ...
        for key_kwargs in keys:
            ...
            protected_checkpoints.update(self.find_checkpoints(max_num_checkpoints=num_to_keep, **key_kwargs))
    ...
    def _call_load_hooks(self, checkpoint, device=None):
        ...
        for name, obj in self.recoverables.items():
            ...
            if name in self.custom_load_hooks:
                self.custom_load_hooks[name](obj, loadpath, end_of_epoch, device)
                continue
            # Otherwise find the default saver for that type:
    ...
    def _list_checkpoint_dirs(self):
        ...
        return [x for x in self.checkpoints_dir.iterdir() if Checkpointer._is_checkpoint_dir(x)]
    ...
    def _new_checkpoint_dirpath(self):
        ...
        suffix_num = 0
        while (self.checkpoints_dir / f"{CKPT_PREFIX}+{stamp}+{suffix_num:02d}").exists():
            suffix_num += 1
...
def average_checkpoints(
    ...
    try:
        ...
        parameter_iterator = (parameter_loader(ckpt.paramfiles[recoverable_name]) for ckpt in checkpoint_list)
