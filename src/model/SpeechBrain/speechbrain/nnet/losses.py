...
def cal_si_snr(source, estimate_source):
    ...
    source_lengths = torch.tensor([estimate_source.shape[0]] * estimate_source.shape[-2], device=device)
...
def cal_snr(source, estimate_source):
    ...
    snr_beforelog = torch.sum(s_target**2, dim=0) / (torch.sum((s_estimate - s_target) ** 2, dim=0) + EPS)
...
def nll_loss_kd(
    ...
    # Generating mask
    mask = length_to_mask(lab_lengths, max_len=max_len, dtype=torch.float, device=prob_curr.device)
