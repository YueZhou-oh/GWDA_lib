...
@cuda.jit("(float32[:,:,:,:], int32[:,:], float32[:,:,:], float32[:], int32[:], int32[:], int32, int32[:,:])")
def cu_kernel_forward(log_probs, labels, alpha, log_p, T, U, blank, lock):
    ...
    if u <= U[b]:
        ...
        while t < T[b]:
            if u == 0:
                ...
                if cuda.atomic.add(lock, (b, u), 0) < 0:
                    if t == 0:
                        alpha[b, 0, u] = alpha[b, 0, u - 1] + log_probs[b, 0, u - 1, labels[b, u - 1]]
                    else:
                        # compute emission prob
                        emit = alpha[b, t, u - 1] + log_probs[b, t, u - 1, labels[b, u - 1]]
                        # compute no_emission prob
                        no_emit = alpha[b, t - 1, u] + log_probs[b, t - 1, u, blank]
                        # do logsumexp between log_emit and log_no_emit
                        alpha[b, t, u] = max(no_emit, emit) + math.log1p(math.exp(-abs(no_emit - emit)))
                    if u < U[b]:
                        cuda.atomic.add(lock, (b, u + 1), -1)
        ...
        if u == U[b]:
            ...
            log_p[b] = (alpha[b, T[b] - 1, U[b]] + log_probs[b, T[b] - 1, U[b], blank]) / T[b]
        ...
        while t >= 0:
            if u == U[b]:
                ...
                if cuda.atomic.add(lock, (b, u), 0) < 0:
                    if t == T[b] - 1:
                        # do logsumexp between log_emit and log_no_emit
                        beta[b, t, u] = beta[b, t, u + 1] + log_probs[b, t, u, labels[b, u]]
                    else:
                        # compute emission prob
                        ...
                        # do logsumexp between log_emit and log_no_emit
                        beta[b, t, u] = max(no_emit, emit) + math.log1p(math.exp(-abs(no_emit - emit)))
                    if u > 0:
                        cuda.atomic.add(lock, (b, u - 1), -1)
                    cuda.atomic.add(lock, (b, u), 1)
                    t -= 1
...
@cuda.jit("(float32[:,:,:,:], int32[:,:],float32[:,:,:], float32[:,:,:], float32[:,:,:,:], int32[:], int32[:], int32)")
def cu_kernel_compute_grad(log_probs, labels, alpha, beta, grads, T, U, blank):
    ...
    if t < T[b]:
        ...
        if t == 0:
            grads[b, T[b] - 1, U[b], blank] = -math.exp(alpha[b, T[b] - 1, U[b]] + log_probs[b, T[b] - 1, U[b], blank] - beta[b, 0, 0])

        if t < T[b] - 1:
            for u in range(U[b] + 1):
                grads[b, t, u, blank] = alpha[b, t, u] + beta[b, t + 1, u]
                grads[b, t, u, blank] = -math.exp(grads[b, t, u, blank] + log_probs[b, t, u, blank] - beta[b, 0, 0])
        # compute the gradient for emit prob
        for u, l in enumerate(labels[b]):
            if u < U[b]:
                grads[b, t, u, l] = alpha[b, t, u] + beta[b, t, u + 1]
                grads[b, t, u, l] = -math.exp(grads[b, t, u, l] + log_probs[b, t, u, l] - beta[b, 0, 0])
