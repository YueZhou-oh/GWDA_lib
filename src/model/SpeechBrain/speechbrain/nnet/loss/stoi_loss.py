...
def stoi_loss(y_pred_batch, y_true_batch, lens, reduction="mean"):
    ...
    for i in range(0, batch_size):  # Run over mini-batches
        ...
        stft_true = torchaudio.transforms.Spectrogram(n_fft=512, win_length=256, hop_length=128, power=2)(y_sil_true)
        stft_pred = torchaudio.transforms.Spectrogram(n_fft=512, win_length=256, hop_length=128, power=2)(y_sil_pred)
        ...
        alpha = torch.norm(X, dim=-1, keepdim=True) / (torch.norm(Y, dim=-1, keepdim=True) + smallVal)
