...
class LinearScheduler:
    ...
    def __init__(self, initial_value, final_value, epoch_count):
        self.value_at_epoch = torch.linspace(initial_value, final_value, steps=epoch_count).tolist()
...
class LinearWarmupScheduler:
    ...
    * Ge Li 2022
    ...
    def calculate_lr(self, current_step):
        ...
        return self.lr0 * max(
            0.0,
            float(self.num_training_steps - current_step) / float(max(1, self.num_training_steps - self.num_warmup_steps)),
        )
...
class StepScheduler:
    ...
    def __init__(self, initial_value, decay_factor=None, decay_drop=None, half_life=None):
        self.initial_value = initial_value
        if half_life:
            if decay_factor or decay_drop:
                raise ValueError("half_life cannot be used together with decay_factor and decay_drop")
...
class NoamScheduler:
    ...
    def _get_lr_scale(self):
        ...
        return self.normalize * min(n_steps ** (-0.5), n_steps * n_warmup_steps ** (-1.5))
        ...
        for opt in optim_list:
            ...
            if current_epoch <= self.dont_halve_until_epoch:
                ...
                if current_loss <= self.anchor:
                    ...
                    next_lr = current_lr
                    self.anchor = current_loss
                elif current_loss > self.anchor and self.patience_counter < self.patience:
...
class IntervalScheduler:
    ...
    def _compute_next(self):
        self._next_intervals = [interval for interval in self.intervals if interval["steps"] > self.n_steps]
