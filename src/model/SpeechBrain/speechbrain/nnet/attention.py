...
class RelPosEncXL(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            pe_past = tot_pe[0]
            pe_future = tot_pe[1]
            positions = torch.arange(0, seq_len, dtype=x.dtype, device=x.device).to(x).unsqueeze(-1)
...
class RelPosMHAXL(nn.Module):
    ...
    def forward(
        ...
        x = x.transpose(1, 2).contiguous().view(bsz, -1, self.vhead_dim * self.num_heads)  # (batch, time1, d_model)
