    ...
    try:
        ...
        if sb.utils.distributed.if_main_process():
            ...
            if hyperparams_to_save is not None:
                hyperparams_filename = os.path.join(experiment_directory, "hyperparams.yaml")
...
def parse_arguments(arg_list=None):
    ...
    parser.add_argument(
        "param_file",
        type=str,
        help="A yaml-formatted file using the extended YAML syntax. " "defined by SpeechBrain.",
    )
    parser.add_argument(
        "--debug",
        default=False,
        action="store_true",
        help="Run the experiment with only a few batches for all " "datasets, to ensure code runs without crashing.",
    )
    parser.add_argument(
        "--debug_batches",
        type=int,
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="This flag enables training with DDP. Assumes script run with " "`torch.distributed.launch`",
    )
    parser.add_argument(
        "--distributed_backend",
        type=str,
    ...
    parser.add_argument(
        "--max_grad_norm",
        type=float,
        help="Gradient norm will be clipped to this value, " "enter negative value to disable.",
    )
    parser.add_argument(
        "--nonfinite_patience",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        help="Amount of time between saving intra-epoch checkpoints " "in minutes. If non-positive, intra-epoch checkpoints are not saved.",
    )
    parser.add_argument(
...
class Brain:
    ...
    * ``evaluate_batch()``
    ...
    def __init__(  # noqa: C901
        ...
        for arg, default in run_opt_defaults.items():
            if run_opts is not None and arg in run_opts:
                if hparams is not None and arg in hparams:
                    logger.info("Info: " + arg + " arg overridden by command line input to: " + str(run_opts[arg]))
        ...
        if not (sys.version_info.major == PYTHON_VERSION_MAJOR and sys.version_info.minor >= PYTHON_VERSION_MINOR):
            logger.warn("Detected Python " + str(sys.version_info.major) + "." + str(sys.version_info.minor) + ". We suggest using SpeechBrain with Python >=" + str(PYTHON_VERSION_MAJOR) + "." + str(PYTHON_VERSION_MINOR))
        ...
        if self.debug and self.checkpointer is not None and hasattr(self.checkpointer, "checkpoints_dir"):
            tempdir = tempfile.TemporaryDirectory()
            logger.info("Since debug mode is active, switching checkpointer " f"output to temporary directory: {tempdir.name}")
        ...
        total_params = sum(p.numel() for p in self.modules.parameters() if p.requires_grad)
        if total_params > 0:
        ...
        if self.distributed_launch:
            ...
            if not torch.distributed.is_initialized():
                if self.rank > 0:
                    ...
                    logger.warn("To use DDP, please add " "sb.utils.distributed.ddp_init_group() into your exp.py")
                    logger.info("Only the main process is alive, " "all other subprocess were killed.")
    ...
    def make_dataloader(self, dataset, stage, ckpt_prefix="dataloader-", **loader_kwargs):
        ...
        **loader_kwargs : dict
        ...
        if self.checkpointer is not None and ckpt_prefix is not None and (isinstance(dataloader, SaveableDataLoader) or isinstance(dataloader, LoopedLoader)):
    ...
    def _train_loader_specifics(self, dataset, loader_kwargs):
        ...
        if self.distributed_launch and not isinstance(dataset, IterableDataset):
            ...
            logger.warning("Cannot automatically solve distributed sampling " "for IterableDataset.")
    ...
    def check_gradients(self, loss):
        ...
        if not torch.isfinite(loss):
            ...
            if self.nonfinite_count > self.nonfinite_patience:
                raise ValueError("Loss is not finite and patience is exhausted. " "To debug, wrap `fit()` with " "autograd's `detect_anomaly()`, e.g.\n\nwith " "torch.autograd.detect_anomaly():\n\tbrain.fit(...)")
            else:
        ...
        if self.max_grad_norm > 0.0:
            torch.nn.utils.clip_grad_norm_((p for p in self.modules.parameters()), self.max_grad_norm)
    ...
    def _compile_jit(self):
        ...
        for name in self.jit_module_keys:
            if name not in self.modules:
                raise ValueError("module" + name + " is not defined in your hparams file.")
    ...
    def evaluate(
        ...
        with torch.no_grad():
            ...
            run_on_main(self.on_stage_end, args=[Stage.TEST, avg_test_loss, None])
        self.step = 0
    ...
    def no_sync(self, use=True):
        ...
        if use:
            ...
            yield
            for module, old_value in zip(self.modules.values(), old_values_list):
                if not hasattr(module, "require_backward_grad_sync"):
                    break
