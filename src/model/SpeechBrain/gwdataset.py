...
class GW_SE_Dataset(object):
    def __init__(
        ...
        if use_PyCBC or use_NRSur:
            ...
            self.param_idx = param_idx
            self.nparams = nparams
            self.bbh_par_range = dict(M=[6, 8], q=[0.01, 1], spin1z=[-0.99, 0.99], spin2z=[-0.99, 0.99])
        ...
        self.buffer = 1000
        self.time_duration = (length + 2 * self.buffer) / self.sampling_rate  # [s]   #3600.0*24*3
    ...
    def init_AAK_waveform_generator(self, use_gpu=True):
        ...
        self.AAK = Pn5AAKWaveform(inspiral_kwargs=inspiral_kwargs, sum_kwargs=sum_kwargs, use_gpu=use_gpu)
    ...
    def get_psd(self, fname="LISA_PSD_2.txt"):
        psd = pycbc.psd.read.from_txt(fname, self.Nf, self.delta_f, self.f_min, is_asd_file=False)
        return psd.data
    ...
    def LISA_fpfc(self, theta_S_bar, phi_S_bar, phi_0_bar=0):
        ...
        cos_theta_S = 0.5 * cos_theta_S_bar - 0.5 * np.sqrt(3) * sin_theta_S_bar * cos_phi_S_bar
        ...
        psi_S = np.arctan2(
            (np.sqrt(3) * np.cos(phi_t_bar) + 2 * sin_theta_S_bar * cos_theta_S_bar * cos_phi_S_bar),
    ...
    def get_sgwb_psd(self, alpha=-11.352, n_t=2 / 3, f_star=1e-3):
        ...
        psd[fidx1:fidx2] = (h2_Omega_gw[fidx1:fidx2]) / (self.sample_frequencies[fidx1:fidx2] ** 3)
        psd_tmp = a * h**2 * psd
        return psd_tmp
    ...
    def generate_emri_dataset(self, n_grid=[4, 3]):
        ...
        self.waveform_dataset["train"]["clean"] = np.zeros([int(waveform_num), self.Nt - 2 * self.buffer])
        self.waveform_dataset["train"]["noisy"] = np.zeros([int(waveform_num), self.Nt - 2 * self.buffer])
        self.waveform_par["train"] = np.zeros([int(waveform_num), self.nparams])
        for i, par_idx in tqdm(enumerate(itertools.product(range(grid_per_par), repeat=self.nparams))):
        ...
        waveform_num = grid_per_par**self.nparams
        self.waveform_dataset["test"]["clean"] = np.zeros([int(waveform_num), self.Nt - 2 * self.buffer])
        self.waveform_dataset["test"]["noisy"] = np.zeros([int(waveform_num), self.Nt - 2 * self.buffer])
    ...
    def generate_bwd_dataset(self, n_grid=[4, 3, 11, 7]):
        ...
        for i, par_idx in tqdm(enumerate(itertools.product(range(grid_per_par), repeat=nparams))):
        ...
        dpd2 = (self.BWD_par_range["f1"][1] - self.BWD_par_range["f1"][0]) / (par1.shape[1]) / 2
        par1[0, :] = np.linspace(
        ...
        dpd2 = (self.BWD_par_range["f_dot1"][1] - self.BWD_par_range["f_dot1"][0]) / (par1.shape[1]) / 2
        par1[1, :] = np.linspace(
        ...
        dpd2 = (self.BWD_par_range["f2"][1] - self.BWD_par_range["f2"][0]) / (par1.shape[1]) / 2
        par2[0, :] = np.linspace(
        ...
        dpd2 = (self.BWD_par_range["f_dot2"][1] - self.BWD_par_range["f_dot2"][0]) / (par1.shape[1]) / 2
        par2[1, :] = np.linspace(
    ...
    def generate_noise_dataset(self, n_train=5e4, n_test=1e4):
        ...
        """
        # training set
        self.waveform_dataset["train"]["clean"] = np.zeros([int(n_train), self.Nt - 2 * self.buffer])
        self.waveform_dataset["train"]["noisy"] = np.zeros([int(n_train), self.Nt - 2 * self.buffer])
        self.waveform_par["train"] = np.zeros([int(n_train), 4])
        for i in tqdm(range(int(n_train + 0.5))):
            noise = self.gen_noise()
        ...
        # _____________________________
        # test set
        self.waveform_dataset["test"]["clean"] = np.zeros([int(n_test), self.Nt - 2 * self.buffer])
        self.waveform_dataset["test"]["noisy"] = np.zeros([int(n_test), self.Nt - 2 * self.buffer])
        self.waveform_par["test"] = np.zeros([int(n_test), 4])
    ...
    def generate_smbhb_dataset(self, n_grid=[4, 3]):
        ...
        par = np.zeros([self.nparams, grid_per_par])
        for pkey, idx in self.param_idx.items():
            dpd2 = (self.bbh_par_range[pkey][1] - self.bbh_par_range[pkey][0]) / (par.shape[1]) / 2
            par[idx, :] = np.linspace(
