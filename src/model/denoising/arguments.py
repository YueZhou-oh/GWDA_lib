...
#   https://github.com/NVIDIA/Megatron-LM/blob/v2.5/megatron/arguments.py
...
def parse_args(extra_args_provider=None, defaults={}, ignore_unknown_args=False):
    """Parse all arguments."""
    parser = argparse.ArgumentParser(description="Megatron-GW Arguments", allow_abbrev=False)
    ...
    assert args.world_size % args.tensor_model_parallel_size == 0, "world size" " ({}) is not divisible by tensor model parallel size ({})".format(args.world_size, args.tensor_model_parallel_size)
    # Pipeline model parallel size.
    args.pipeline_model_parallel_size = min(args.pipeline_model_parallel_size, (args.world_size // args.tensor_model_parallel_size))
    # Checks.
    model_parallel_size = args.pipeline_model_parallel_size * args.tensor_model_parallel_size
    assert args.world_size % model_parallel_size == 0, "world size is not" " divisible by tensor parallel size ({}) times pipeline parallel " "size ({})".format(args.world_size, args.tensor_model_parallel_size, args.pipeline_model_parallel_size)
    args.data_parallel_size = args.world_size // model_parallel_size
    if args.rank == 0:
        print(
            "using world size: {}, data-parallel-size: {}, "
            "tensor-model-parallel size: {}, "
            "pipeline-model-parallel size: {} ".format(args.world_size, args.data_parallel_size, args.tensor_model_parallel_size, args.pipeline_model_parallel_size),
            flush=True,
        )

    # Deprecated arguments
    assert args.batch_size is None, "--batch-size argument is no longer " "valid, use --micro-batch-size instead"
    del args.batch_size
    assert args.warmup is None, "--warmup argument is no longer valid, use " "--lr-warmup-fraction instead"
    del args.warmup
    assert args.model_parallel_size is None, "--model-parallel-size is no " "longer valid, use --tensor-model-parallel-size instead"
    ...
    if args.num_layers_per_virtual_pipeline_stage is not None:
        assert args.pipeline_model_parallel_size > 2, "pipeline-model-parallel size should be greater than 2 with " "interleaved schedule"
        assert args.num_layers % args.num_layers_per_virtual_pipeline_stage == 0, "number of layers is not divisible by number of layers per virtual " "pipeline stage"
        args.virtual_pipeline_model_parallel_size = (args.num_layers // args.pipeline_model_parallel_size) // args.num_layers_per_virtual_pipeline_stage
    else:
...
def _add_network_size_args(parser):
    ...
    group.add_argument("--openai-gelu", action="store_true", help="Use OpenAIs GeLU implementation. This option" "should not be used unless for backward compatibility" "reasons.")
    group.add_argument("--onnx-safe", type=bool, required=False, help="Use workarounds for known problems with " "Torch ONNX exporter")
    group.add_argument("--no-binary-head", action="store_false", help="Disable binary head.", dest="binary_head")
...
def _add_regularization_args(parser):
    ...
    group.add_argument("--adam-eps", type=float, default=1e-08, help="Term added to the denominator to improve" "numerical stability of adam")
    group.add_argument("--lamb-eps", type=float, default=1e-06, help="Term added to the denominator to improve" "numerical stability of lamb")
...
def _add_training_args(parser):
    ...
    group.add_argument("--distribute-checkpointed-activations", action="store_true", help="If set, distribute checkpointed activations " "across model parallel group.")
    ...
    group.add_argument("--optimizer", type=str, default="adam", choices=["adam", "sgd", "lamb"], help="Optimizer function")
    group.add_argument("--dataloader-type", type=str, default=None, choices=["single", "cyclic"], help="Single pass vs multiple pass data loader")
    return parser
...
def _add_learning_rate_args(parser):
    ...
    group.add_argument("--lr", type=float, default=None, help="Initial learning rate. Depending on decay style " "and initial warmup, the learing rate at each " "iteration would be different.")
    group.add_argument("--lr-decay-style", type=str, default="linear", choices=["constant", "linear", "cosine"], help="Learning rate decay function.")
    ...
    group.add_argument("--min-lr", type=float, default=0.0, help="Minumum value for learning rate. The scheduler" "clip values below this threshold.")
    group.add_argument(
...
def _add_mixed_precision_args(parser):
    ...
    group.add_argument("--min-loss-scale", type=float, default=1.0, help="Minimum loss scale for dynamic loss scale.")
    ...
    group.add_argument("--no-query-key-layer-scaling", action="store_false", help="Do not scale Q * K^T by 1 / layer-number.", dest="apply_query_key_layer_scaling")
    ...
    group.add_argument("--fp16-lm-cross-entropy", action="store_true", help="Move the cross entropy unreduced loss calculation" "for lm head to fp16.")
...
def _add_distributed_args(parser):
    ...
    group.add_argument("--DDP-impl", default="local", choices=["local", "torch"], help="which DistributedDataParallel implementation " "to use.")
    group.add_argument("--use-contiguous-buffers-in-ddp", action="store_true", help="If set, use contiguous buffer in DDP. Note that " "this option only works woth local DDP.")
    group.add_argument("--no-scatter-gather-tensors-in-pipeline", action="store_false", help="Use scatter/gather to optimize communication of tensors in pipeline", dest="scatter_gather_tensors_in_pipeline")
    ...
    group.add_argument("--use-cpu-initialization", action="store_true", default=None, help="If set, affine parallel weights " "initialization uses CPU")
    return parser
...
def _add_validation_args(parser):
    ...
    group.add_argument("--eval-iters", type=int, default=1, help="Number of iterations to run for evaluation" "validation/test for.")
...
def _add_data_args(parser):
    ...
    group.add_argument("--segment-length", type=int, default=2048, help="Length of signal patch.")
    group.add_argument("--data-path", nargs="*", default=None, help="Path to the training dataset. Accepted format:" "1) a single data path, 2) multiple datasets in the" "form: dataset1-weight dataset1-path dataset2-weight " "dataset2-path ...")
    group.add_argument(
        "--split", type=str, default="969, 30, 1", help="Comma-separated list of proportions for training," " validation, and test split. For example the split " "`90,5,5` will use 90%% of data for training, 5%% for " "validation and 5%% for test."
    )
    ...
    group.add_argument("--dets", type=str, default="H1", help="detections that are used to generate data.")
    group.add_argument("--encoder-seq-length", type=int, default=None, help="Maximum encoder sequence length to process." "This should be exclusive of --seq-length")
    group.add_argument("--decoder-seq-length", type=int, default=None, help="Maximum decoder sequence length to process.")
    group.add_argument("--retriever-seq-length", type=int, default=256, help="Maximum sequence length for the biencoder model " " for retriever")
    group.add_argument("--sample-rate", type=float, default=1.0, help="sample rate for training data. Supposed to be 0 " " < sample_rate < 1")
    group.add_argument("--mask-prob", type=float, default=0.15, help="Probability of replacing a token with mask.")
    ...
    group.add_argument("--data-impl", type=str, default="infer", choices=["lazy", "cached", "mmap", "infer"], help="Implementation of indexed datasets.")
...
def _add_autoresume_args(parser):
    ...
    group.add_argument("--adlr-autoresume-interval", type=int, default=1000, help="Intervals over which check for autoresume" "termination signal")
...
def _add_biencoder_args(parser):
    ...
    group.add_argument("--biencoder-projection-dim", type=int, default=0, help="Size of projection head used in biencoder (paper" " default: 128)")
    group.add_argument("--biencoder-shared-query-context-model", action="store_true", help="Whether to share the parameters of the query " "and context models or not")
    ...
    group.add_argument("--evidence-data-path", type=str, default=None, help="Path to Wikipedia Evidence frm DPR paper")

    # training
    group.add_argument("--retriever-report-topk-accuracies", nargs="+", type=int, default=[], help="Which top-k accuracies to report " "(e.g. '1 5 20')")
    group.add_argument("--retriever-score-scaling", action="store_true", help="Whether to scale retriever scores by inverse " "square root of hidden size")

    # faiss index
    group.add_argument("--block-data-path", type=str, default=None, help="Where to save/load BlockData to/from")
    group.add_argument("--embedding-path", type=str, default=None, help="Where to save/load Open-Retrieval Embedding" " data to/from")
...
def _add_vit_args(parser):
    ...
    group.add_argument("--num-classes", type=int, default=1000, help="num of classes in vision classificaiton task")
    group.add_argument("--img-dim", type=int, default=224, help="Image size for vision classification task")
    group.add_argument("--num-channels", type=int, default=3, help="Number of channels in input image data")
    group.add_argument("--patch-dim", type=int, default=16, help="patch dimension used in vit")
