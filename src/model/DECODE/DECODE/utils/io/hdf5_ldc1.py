...
from emridetection.utils.lisa.LISACommonFunctions import *
...
class ParsUnits:
    ...
    def __init__(self, pars_i=None, units_i=None, name="", value=0.0, unit=""):
        """
        Initalize `ParsUnits`
    ...
    def getConvert(self, parName, conversion, requiredUnit):
        ...
        if type(v) != type(np.zeros(10)) and v == None:
            ...
            if requiredUnit not in conversion:
                print(
                    "WARNING: ParsUnits.getConvert : parameter unit",
                    requiredUnit,
                    "is not in the conversion list (",
                    conversion,
                    ")",
                )
                return None
            if uV not in conversion:
                print(
                    "WARNING: ParsUnits.getConvert : required unit",
                    uV,
                    "is not in the conversion list (",
                    conversion,
                    ")",
...
class LISAhdf5:
    ...
    def __init__(self, filename, author="Stas Babak, Antoine Petiteau (2017)", mode="a"):
        """
        Create hdf5 file and create the main group
        @param filename: name of hdf5 file
        """
        self.filename = filename
        self.h5file = h5.File(self.filename, mode=mode)
        self.MainGroupName = "H5LISA"
        self.MainGroup = self.h5file.require_group(self.MainGroupName)
        if not "Author" in self.MainGroup and mode in ["a", "w"]:
    ...
    def compare(self, ref, verbose=False):
        ...
        if len(l_loc) != len(l_ref):
            if verbose:
                print(
                    "LISAhdf5.compare : different number object: ",
                    len(l_loc),
                    "vs",
                    len(l_ref),
                )
        ...
        for i in range(len(l_loc)):
            ...
            if ll != lr and (re.search("H5LISA/History", ll) == None):
                if verbose:
                    print(
                        "LISAhdf5.compare : difference in line ",
                        i,
                        ": ",
                        len(ll),
                        "vs",
            ...
            if x[:14] != "H5LISA/History":
                ...
                if type(g0) != type(p) and len(p.shape) != 0:
                    ...
                    if len(ib) != 0:
                        if verbose:
                            print(
                                "LISAhdf5.compare : difference for array ",
                                x,
                                "at index",
                                ib,
                            )
    ...
    def addSource(
        ...
        try:
            ...
            if overwrite:
                ...
                print(
                    "ERROR: LISAhdf5:addSource : Cannot overwrite an existing source ",
                    sourceName,
                    "!",
                )
                sys.exit(1)
    ...
    def getSourceParameters(self, SrcName):
        ...
        if SrcName not in SrcNames:
            print(
                "WARNING: LISAhdf5.getSource: ",
                SrcName,
                "is not in source names",
                SrcNames,
                "!",
            )
        else:
            ...
            for ky in list(src.keys()):
                if ky != "hphcData" and ky != "XYZdata":
                    p = src.get(ky)
                    params.addPar(ky, p[()], Str(p.attrs["Units"]))
        self.h5file.close()
        return params
    ...
    def getSourceHpHc(self, SrcName):
        ...
        if SrcName not in SrcNames:
            ...
            if "hphcData" in list(src.keys()):
                ...
                print(
                    "ERROR in LISAhdf5:getSourceHpHc: there is no h+, hc data for the source",
                    SrcName,
                )
                sys.exit(1)
        self.h5file.close()
    ...
    def addLISADataSource(self, Name, Model, parameters, overwrite=False, data=None):
        ...
        try:
            ...
            if overwrite:
                ...
                print(
                    "ERROR: LISAhdf5:addLISADataSource : Cannot overwrite an existing data source ",
                    Name,
                    "!",
                )
                sys.exit(1)
    ...
    def addLISADataModel(
        ...
        try:
            ...
            if overwrite:
                ...
                print(
                    "ERROR: LISAhdf5:addLISADataModel : Cannot overwrite an existing data model ",
                    Name,
                    "!",
                )
                sys.exit(1)
        ...
        try:
            ...
            if overwrite:
                ...
                print(
                    "ERROR: LISAhdf5:addLISADataModel : Cannot overwrite an existing input ",
                    pathGroupInputs,
                    "!",
                )
                sys.exit(1)
        ...
        try:
            ...
            if overwrite:
                ...
                print(
                    "ERROR: LISAhdf5:addLISADataModel : Cannot overwrite an existing ouput ",
                    pathGroupOutputs,
                    "!",
                )
                sys.exit(1)
    ...
    def getPreProcessTDI(self):
        ...
        try:
            ...
            if "H5LISA/PreProcess/TDIdata" in list_of_names:
                d = np.copy(self.h5file["H5LISA/PreProcess/TDIdata"])
                self.h5file.close()
                return d
            else:
    ...
    def addUserRequest(self, Name, text, overwrite=True):
        ...
        try:
            ...
            if overwrite:
                ...
                print(
                    "ERROR: LISAhdf5:addUserRequest : Cannot overwrite an existing user request",
                    pathGroupUserRequest,
                    "!",
                )
                sys.exit(1)
